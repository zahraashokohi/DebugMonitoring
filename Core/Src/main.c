...
// فعال‌سازی Debug Monitor
bool debug_monitor_enable(void) {
    // اگر halting debug فعال باشه، کار نمی‌کنه
    if (CoreDebug->DHCSR & CoreDebug_DHCSR_C_DEBUGEN_Msk) {
        return 0;
    }

    // فعال کردن DebugMonitor
...
void Enable_DWT_Watchpoint(void)
{
  ...
  // فعال کردن Debug Monitor
  CoreDebug->DEMCR |= CoreDebug_DEMCR_MON_EN_Msk;

  // تنظیم Watchpoint روی test_var
  DWT->COMP0 = (uint32_t)&test_var;   // آدرس متغیر
  DWT->MASK0 = 0;                     // فقط همون آدرس
  DWT->FUNCTION0 = 0x5;               // watchpoint on write        // Watchpoint on write
}
/* USER CODE END PV */
